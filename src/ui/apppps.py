...
floating_chatbot.render_chatbot()
...
with tab_chat:
    ...
    # Conserver la valeur de top_k côté session pour l'API /chat (optionnel)
    st.session_state["top_k_from_ui"] = top_k

    q = st.text_input("Ask a question about your indexed documents", key="q_chat_input")
    c1, c2 = st.columns([1,1])
    do_search = c1.button("🔎 Retrieve & Answer", use_container_width=True, key="btn_chat_search")
    clear_box = c2.button("🧹 Clear", use_container_width=True, key="btn_chat_clear")
    ...
    if last:
        ...
        if last.get("suggestions"):
            st.info("I couldn't find a precise answer. Try one of these:")
            sug_choice = st.radio(
                "Suggestions",
                last["suggestions"],
                key="q_suggestion_radio"
            )
            if st.button("✅ Ask this suggestion", key="btn_chat_suggest"):
                with st.spinner("Retrieving context & generating answer for suggestion..."):
                    new_res = ask_rag(
                        question=sug_choice,
                        k=top_k,
                        get_sources_fn=_get_source_names_for_query
...
with tab_upload:
    ...
    files = st.file_uploader(t("upload_uploader"), type=["pdf", "docx", "txt", "md"], accept_multiple_files=True)
    c1, c2 = st.columns(2)
    if c1.button(t("upload_save"), use_container_width=True) and files:
        saved = save_uploaded_files(files)
        st.success(t("upload_saved").format(n=len(saved)))
    if c2.button(t("upload_extract"), use_container_width=True):
        with st.spinner("Extracting & chunking..." if st.session_state["lang"] == "en" else "Extraction & segmentation..."):
            n, outp = extract_and_chunk()
        st.success(t("upload_extracted").format(n=n, outp=outp))
    if st.button(t("upload_rebuild"), use_container_width=True):
        with st.spinner("Encoding & indexing..." if st.session_state["lang"] == "en" else "Encodage & indexation..."):
            total = rebuild_index()
        st.success(t("upload_rebuilt").format(n=total))

    if (DATA_PROCESSED / "chunks.jsonl").exists():
        st.markdown("#### " + t("upload_preview"))
...
with tab_qa:
    rag_en_tab.render()
# Appel unique pour rendre la bulle + panneau partout
    
# ---- TAB 6: TRANSLATION (outil de traduction de texte, indépendant de l'i18n UI) ----
from googletrans import Translator

with st.expander("🔤 Traduction automatique", expanded=False):
    st.subheader("Traduction multilingue (Google Translate)")
    ...
    src_label = st.selectbox("Langue source (Auto = détection)", list(languages.keys()), index=0, key="src_lang")
    tgt_label = st.selectbox("Langue cible", list(languages.keys())[1:], index=1, key="tgt_lang")

    src_code = languages[src_label]
    tgt_code = languages[tgt_label]

    texte = st.text_area("Texte à traduire", "", key="translation_input")

    if st.button("Traduire", key="translate_btn"):
        if texte.strip():
            try:
                result = translator.translate(texte, src=src_code, dest=tgt_code)
                st.success("Traduction : " + result.text)
            except Exception as e:
                st.error(f"Erreur de traduction : {e}")
        else:
            st.warning("Veuillez saisir un texte à traduire.")
# --------------------------------------------------------------------------------------
# END OF FILE
# --------------------------------------------------------------------------------------