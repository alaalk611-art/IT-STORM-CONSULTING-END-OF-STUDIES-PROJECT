...
<div style="font-family: system-ui, -apple-system, Segoe UI, Roboto, Arial;">
  <style>
    ...
    .vu-txt {{
      font-weight:950; font-size:12px; width:120px; text-align:right;
      color: var(--text);
    }}
    ...
    /* Hidden debug panel (kept for dev, not for UI) */
    .hidden {{
...
(function() {{
  ...
  let isBusy = false;
  let isSpeaking = false; // ✅ important
  let silenceCount = 0;
  let recStartedAt = 0;
  ...
  function startVu(s) {{
    try {{
      ...
      vuTimer = setInterval(() => {{
        ...
        // ✅ block auto-start while TTS is playing (prevents self-trigger)
  ...
  async function autoPipeline() {{
    ...
    try {{
      ...
      // ✅ after speaking ends, callUi becomes Connected via onended
      isBusy = false;
      return;
